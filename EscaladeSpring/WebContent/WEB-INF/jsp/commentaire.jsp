				...
				<h2>Pour les amis de l'escalade</h2>	
					...
					<table class="table">
						...
						<c:forEach var="commentaire" items="${ commentaire }">
							<tr>
								...
								<c:if test = "${current == 'ShieldDirector@gmail.com'}">
									<td><a href="${contextPath}/modifyComment/${commentaire.id}">Modifier</a></td>
									<td><a href="${contextPath}/supprComment/${commentaire.id}">Supprimer</a></td>
								</c:if>
							</tr>
						</c:forEach>
					</table>
					<div class="content-button">
						<c:if test = "${current != 'anonymousUser'}">
							<a href="${contextPath}/addComments/${id}">Ajouter commentaire</a>
						</c:if>
