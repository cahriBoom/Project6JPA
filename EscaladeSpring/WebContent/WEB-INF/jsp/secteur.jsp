				...
				<div align="center" class="table-mg">
					<table class="table">
						...
						<c:forEach var="secteur" items="${ secteur }">
							<tr>
								<td><a href ="${contextPath}/description/${ secteur.id }"><c:out value="${ secteur.nom }"></c:out></a></td>
					...
					<div class="content-button">
						<c:if test = "${current != 'anonymousUser'}">
							<a href="${contextPath}/addSecteur/${id}">Ajouter Secteur</a>
						</c:if>
						<a href="${contextPath}/liste">Retour Site</a>
					</div>
					<div class="content-button">
						<c:if test = "${current == 'ShieldDirector@gmail.com'}">
							<a href="${contextPath}/validationOfficial/${id}">Site Officiel "Amis de l'escalade"</a>
						</c:if>
					</div>
				</div>
			</div>
